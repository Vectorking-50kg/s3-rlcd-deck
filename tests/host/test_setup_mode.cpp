...
void http_contract_exposes_profile_pairing_without_secrets()
{
    ...
    assert(routes[1].route == DECK_SETUP_HTTP_LEGACY_PAIRING_PAGE);
    assert(routes[1].method == DECK_SETUP_HTTP_GET);
    assert(std::string(routes[1].path) == "/compat/pairing-v1");
    assert(routes[2].route == DECK_SETUP_HTTP_STATUS);
    assert(routes[2].method == DECK_SETUP_HTTP_GET);
    assert(std::string(routes[2].path) == "/api/status");
    assert(routes[3].route == DECK_SETUP_HTTP_SCAN);
    assert(routes[3].method == DECK_SETUP_HTTP_POST);
    assert(std::string(routes[3].path) == "/api/scan");
    assert(routes[4].route == DECK_SETUP_HTTP_WIFI);
    assert(routes[4].method == DECK_SETUP_HTTP_POST);
    assert(std::string(routes[4].path) == "/api/wifi");
    assert(routes[5].route == DECK_SETUP_HTTP_TEMPERATURE);
    assert(std::string(routes[5].path) == "/api/temperature");
    assert(routes[6].route == DECK_SETUP_HTTP_WIFI_CLEAR_REQUEST);
    assert(std::string(routes[6].path) == "/api/wifi/clear/request");
    assert(routes[7].route == DECK_SETUP_HTTP_WIFI_CLEAR_CONFIRM);
    assert(std::string(routes[7].path) == "/api/wifi/clear/confirm");
    assert(routes[8].route == DECK_SETUP_HTTP_COMPANION_PAIR);
    assert(std::string(routes[8].path) == "/api/companions/pair");
    assert(routes[9].route == DECK_SETUP_HTTP_COMPANION_PAIR_ACK);
    assert(std::string(routes[9].path) == "/api/companions/pair/ack");
    assert(routes[10].route == DECK_SETUP_HTTP_COMPANION_SELECT);
    assert(std::string(routes[10].path) == "/api/companions/select");
    assert(routes[11].route == DECK_SETUP_HTTP_COMPANION_PRIORITY);
    assert(std::string(routes[11].path) == "/api/companions/priority");
    assert(routes[12].route == DECK_SETUP_HTTP_COMPANION_REVOKE);
    assert(std::string(routes[12].path) == "/api/companions/revoke");

    assert(deck_setup_http_route("GET", "/") == DECK_SETUP_HTTP_PAGE);
    assert(deck_setup_http_route("GET", "/compat/pairing-v1") ==
    ...
    assert(html.find("Companion profiles") != std::string::npos);
    assert(html.find("Legacy Pairing v1 compatibility") != std::string::npos);
    ...
    const char *legacy_page = deck_setup_http_legacy_pairing_page();
    assert(legacy_page != nullptr);
    const std::string legacy_html(legacy_page);
    assert(legacy_html.find("Legacy Pairing v1") != std::string::npos);
    assert(legacy_html.find("temporary compatibility") != std::string::npos);
    assert(legacy_html.find("name=hub_address") != std::string::npos);
    assert(legacy_html.find("name=code") != std::string::npos);
    assert(legacy_html.find("/api/companions/pair") != std::string::npos);
    assert(legacy_html.find("/api/companions/pair/ack") != std::string::npos);
    assert(legacy_html.find("innerHTML") == std::string::npos);
