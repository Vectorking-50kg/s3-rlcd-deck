...
void owns_the_frame_until_async_completion()
{
    ...
    panel.done(panel.done_context);
    assert(deck_display_service_poll(display, 111) == DECK_DISPLAY_COMPLETED);
    assert(deck_display_service_submit(display, 112) == DECK_DISPLAY_UNCHANGED);

    const deck_display_metrics_t metrics = deck_display_service_metrics(display);
    assert(metrics.submitted_frames == 1);
    assert(metrics.completed_frames == 1);
...
void counts_a_timeout_once_and_accepts_late_completion()
{
    ...
    assert(deck_display_service_metrics(display).completed_frames == 1);
    deck_display_service_destroy(display);
}
