...
- [字形清单](../../firmware/components/application_ui/assets/m0_glyphs.txt)当前有 **202 个唯一 symbols：199 个基本区汉字，加 `°`、`·` 与 `、`**。16/20 px 各包含这 202 个 symbols 与 95 个可打印 ASCII，共 297 个有效 glyph、另一个保留 descriptor；32 px 只有 95 个 ASCII glyph。
- [16 px 字体](../../firmware/components/application_ui/assets/lv_font_deck_m0_16.c)为 `line_height=19`、`base_line=5`、bitmap 6,046 B；[20 px 字体](../../firmware/components/application_ui/assets/lv_font_deck_ui_20.c)为 `line_height=23`、`base_line=6`、bitmap 9,405 B；[32 px ASCII 字体](../../firmware/components/application_ui/assets/lv_font_deck_ui_32.c)为 `line_height=37`、`base_line=9`、bitmap 3,574 B。
- ESP-IDF 6.0.2 的 Xtensa object 实测三档只读字体数据分别为 **8,942 B、12,301 B、4,426 B，合计 25,669 B**。这里未把 debug section 或 C 源码文本大小当成固件 Flash 成本；release 构建应持续用 object/map 文件校验。
...
使用**当前 202-symbol manifest**、同一个 OTF 和 `lv_font_conv@1.5.3`，只改变 `--size`，本次得到：
...
| 16 | 19 | 5 | 6,046 | **8,942 B（当前 object 实测）** | 中文紧凑正文的最低默认值 |
| 18 | 21 | 5 | 7,493 | 约 10,389 B | 后续正文清晰度 A/B 候选 |
| 20 | 23 | 6 | 9,405 | **12,301 B（当前 object 实测）** | 中文标题、告警、短状态 |

估算包含当前 298 个 descriptor、两个 cmap、202 项 Unicode list 和 font descriptor；bitmap、`line_height` 与 `base_line` 直接来自对应生成物，16/20 px 已由当前 Xtensa object 精确复核。这里最重要的不是每档相差一两 KiB，而是 12–14 px 留给复杂汉字的有效像素网格太小。
...
当前约 199 汉字 manifest 已覆盖明显更多的固定 UI 文案，但仍不能保证任意会话名、设备名或远端内容。产品必须选择并记录一种策略：
...
3. 目前 16/20 px 共用同一 202-symbol manifest，简单且合计约 20.7 KiB；若以后需要缩减 Flash，再从实际调用点自动生成独立 manifest，不能手工维护两份易漂移清单。
...
- 还没有针对三档字体的 object/map Flash 自动预算门；本文记录的 25,669 B 是当前 ESP-IDF 6.0.2 Xtensa object 实测值，后续 release 构建应持续验证；
...
这条路线直接匹配当前 16/20 CJK + 32 ASCII 实现，三档合计约 25.1 KiB 只读数据；同时为 18 px 清晰度 A/B、按用途缩减 20 px manifest 和未来常用汉字 fallback 留出了清晰的演进边界。
