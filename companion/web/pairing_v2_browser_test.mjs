...
async function main() {
  ...
  try {
    ...
    const phaseOne = await evaluate(cdp, `(async()=>{
      ...
      const candidateExpiry=document.querySelector('.pairing-candidate small').textContent;
      ...
      const sessionExpiry=document.querySelector('#pairing-v2-countdown').textContent;
      ...
      return {labels,candidateExpiry,sessionExpiry,codeCleared:input.value==='',stored:sessionStorage.getItem('s3deck.pairing-v2.session-ref')};
    })()`);
    assert.deepEqual(phaseOne.labels, ["S3 RLCD Deck · 同名设备 1", "S3 RLCD Deck · 同名设备 2"]);
    assert.match(phaseOne.candidateExpiry, /^匿名候选将在 \d+ 秒后失效。$/);
    assert.match(phaseOne.sessionExpiry, /^本机会话最多保留 \d+ 秒；请以 Deck 屏幕倒计时为准。/);
    assert.doesNotMatch(phaseOne.sessionExpiry, /^配对窗口剩余/);
