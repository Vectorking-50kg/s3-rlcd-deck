...
The authenticated management API issues codes at `POST /api/v1/pairing/codes`, rotates a device token at `POST /api/v1/devices/{device_id}/rotate`, and revokes trust at `DELETE /api/v1/devices/{device_id}`. A Deck redeems a code once at the rate-limited Device Hub route `POST /api/v1/pairing/redeem`. Management writes require the login session, exact Origin, and CSRF token described above; the redeem response is the only place a plaintext device Token is returned.
